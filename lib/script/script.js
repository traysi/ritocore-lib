  ...
  while (i < tokens.length) {
    ...
    if (_.isUndefined(opcodenum)) {
      ...
      if (opcodenum > 0 && opcodenum < Opcode.OP_PUSHDATA1) {
        ...
        throw new Error('Invalid opcode in script (' + token + '):' + JSON.stringify(str));
      }
    } else if (opcodenum === Opcode.OP_PUSHDATA1 ||
      opcodenum === Opcode.OP_PUSHDATA2 ||
      opcodenum === Opcode.OP_PUSHDATA4) {
...
/**
 * @returns {boolean} true if this is a valid asset output
 */
Script.prototype.isAssetOut = function() {
  return !!(this.chunks.length === 8 &&
    this.chunks[0].opcodenum === Opcode.OP_DUP &&
    this.chunks[1].opcodenum === Opcode.OP_HASH160 &&
    this.chunks[2].buf &&
    this.chunks[2].buf.length === 20 &&
    this.chunks[3].opcodenum === Opcode.OP_EQUALVERIFY &&
    this.chunks[4].opcodenum === Opcode.OP_CHECKSIG &&
    this.chunks[5].opcodenum === Opcode.OP_RVN_ASSET &&
    this.chunks[6].buf &&
    this.chunks[6].buf.length > 0 &&
    this.chunks[7].opcodenum === Opcode.OP_DROP);
};
 ...
 * In the case of a standard RVN_ASSET, return the Asset
 * @returns {Buffer}
 */
Script.prototype.getData = function() {
  ...
  if (this.isAssetOut()) {
    return new Asset(this.chunks[6].buf);
  }
...
Script.types.ASSET_OUT = 'Asset to public key hash';
...
Script.outputIdentifiers.ASSET_OUT = Script.prototype.isAssetOut;
